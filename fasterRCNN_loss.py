...
class FasterRCNN_Loss(caffe.Layer):
	...
	if correct_count[1] + batch_size - pos_count - correct_count[0] == 0:#no positive prediction
        ...
        if False and self.phase== caffe.TEST:#self.phase == 'TEST' or self.iter % 10 == 0:
            ...
            ax = plt.gca()
            plt.imshow(img)        
            resize_width = cfgs.resize_width
            resize_height = cfgs.resize_height
            sliding_window_width = cfgs.sliding_window_width
            sliding_window_height = cfgs.sliding_window_height
            sliding_window_stride = cfgs.sliding_window_stride
            cand_bbs = list()
            cand_probs = list()
